...
def enviar_mensaje(mensaje):
    try:
        ...
        print("Error al conectarse con el API_CENTRAL")
...
def estado_taxi():
    ...
    for message in consumer_taxi_estado:
        print(f"Mensaje recibido: {message.value['id']} - {message.value['Estado']}")
        cursor.execute(f"Select token from Taxi where id = {message.value['id']}")
        token_bd=cursor.fetchone()
        if token_bd[0]==message.value['token']:
            cursor.execute(f"Select destino1 from Taxi where id = {message.value['id']}")
            destino_1=cursor.fetchone()
            if message.value["Estado"] == "KO":
                print(f"Taxi {message.value['id']} fuera de servicio")
                enviar_mensaje(f"Taxi {message.value['id']} fuera de servicio(KO)")
                registro_de_mensajes(f"Taxi {message.value['id']} fuera de servicio(KO)",ip_taxis)
                ...
                cursor.execute(f"Select destino2 from Taxi where id = {message.value['id']}")
                destino2=cursor.fetchone()[0]
                if destino1 is not None:
                    if destino1 in ubicaciones.keys():
                        if destino1 == "Base":
                            print("Servicio asignado Base")
                            enviar_mensaje(f"Servicio asignado Base a taxi {message.value['id']}")
                            registro_de_mensajes(f"Servicio asignado Base a taxi {message.value['id']}",ip_taxis)
                    ...
                    enviar_mensaje(f"Servicio asignado {destino2} a taxi {message.value['id']}")
                    registro_de_mensajes(f"Servicio asignado {destino2} a taxi {message.value['id']}",ip_taxis)
                ...
                imprimir()
            elif message.value["Estado"]== "END" and destino_1[0] is not None:
                ...
                if destino2 is not None:
                    ...
                    cursor.execute("Update Taxi set pasajero = %s where id = %s", (pasajero, message.value['id']))
                ...
                imprimir()
            elif message.value["Estado"]== "END" and destino_1[0] is None:
                cursor.execute(f"Select posx,posy from Taxi where id = {message.value['id']}")
                posx,posy=cursor.fetchone()
                
                print(f"Servicio finalizado {message.value['id']}")
                enviar_mensaje(f"Servicio finalizado {message.value['id']}")
                registro_de_mensajes(f"Servicio finalizado {message.value['id']}",ip_taxis)
                time.sleep(0.5)
                cursor.execute(f"Select pasajero from Taxi where id = {message.value['id']}")
                pasajero=cursor.fetchone()[0]
                cursor.execute(f"Select posx,posy from Taxi where id = {message.value['id']}")
                posx,posy=cursor.fetchone()
                cursor.execute(f"Update Taxi set estado = 'OK' where id = {message.value['id']}")
                ...
                registro_de_mensajes(f"Hace mucho frio, taxi {message.value['id']} fuera de servicio",ip_taxis)
            ...
            print(f"Token incorrecto para taxi {message.value['id']}")
            enviar_mensaje(f"Token incorrecto para taxi {message.value['id']}")
            registro_de_mensajes(f"Token incorrecto para taxi {message.value['id']}",ip_central,"ERROR")
...
def movimientos_taxi():
    ...
    for message in consumer_taxi_movs:
        print(f"Mensaje recibido: {message.value['Movimiento']},"f"ID: {message.value['id']}")
        cursor.execute(f"Select token from Taxi where id = {message.value['id']}")
        token_bd=cursor.fetchone()
        if token_bd[0] == message.value["token"]:
            cursor.execute(f"Select posx,posy from Taxi where id = {message.value['id']}")
            ...
            print(f"Token no válido para el id {message.value['id']}")
            enviar_mensaje(f"Token no válido para el id {message.value['id']}")
            registro_de_mensajes(f"Token no válido para el id {message.value['id']}",ip_central,"ERROR")
