...
print("Clave pública y privada guardadas correctamente")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if current_status == "KO" and last_traffic_status != "KO":
                ...
                cursor.execute("SELECT * FROM Taxi WHERE posx != 1 OR posy != 1 or estado != 'KO' or (posx = 1 and posy = 1 and estado != 'KO')")
                ...
                for taxi in taxis_en_movimiento:
                    ...
                    if estado == "RUN": # Está en movimiento, lo redirigimos a la base
                        ...
                        cursor.execute(
                            "UPDATE Taxi SET estado = 'REDIRECT' WHERE id = %s",
                            (taxi_id,),
                        )
                ...
                for cliente in clientes_con_taxi:
                    cliente_id, posx, posy, destino, estado = cliente
                    cursor.execute("SELECT id FROM Taxi WHERE pasajero = %s", (cliente_id,))
                    taxi_id = cursor.fetchone()
                    taxi_id = taxi_id[0]
                    producer_client.send('Servicios', value={
                        "customer_id": cliente_id,
                        "pickup_location": (posx, posy),
...
def traffic_monitor():
    ...
    while running_CTC:
        try:
            ...
            print(f"[ERROR] Fallo en monitor de tráfico")
        time.sleep(10)
...
def enviar_mensaje(mensaje):
    try:
        ...
        print("Error realizando la solicitud: para enviar mensajes al front")
...
def estado_taxi():
    ...
    for message in consumer_taxi_estado:
        ...
        print(f"Mensaje recibido {clave_aes}")
        if clave_aes is not None:
            ...
            if token_bd[0] == mensaje['token']:
                ...
                if mensaje["Estado"] == "KO":
                    ...
                    registro_de_mensajes(f"Taxi {taxi_id} fuera de servicio(KO)",ip_taxis)
